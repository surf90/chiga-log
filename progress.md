...
- `--icon-wind-arrow` の属性区切り・`viewBox`・path データの空白を `%20` に完全エンコード。
  minify 後の data URI をデコードし、SVG として構文解析できることを確認した。
- `DESIGN.md` に minify 耐性の要件を追記し、同じ記法への退行を防止。
- 追加監査で同じ欠陥が警告・完了・更新アイコンにもあることを確認。3種も同じ方式で修正し、
  回帰テストを全4種のインライン SVG に拡張した。
