...
- **自動データ更新**: GitHub Actionsを利用して定期的にデータを取得・生成することで、APIの制限を回避しつつ、最新の天気予報・潮汐情報・海面気象データを安定して配信しています（BFF構成）。
- **警報フローティングバー**: 気象庁から警報・特別警報が発令された際、画面下部に固定表示される警告バーが出現します（警報=赤、特別警報=紫）。
- **波グラフY軸固定**: 横スクロール中も左右のY軸（波高・周期）が画面端に常時表示されます。
...
### 海面状況 / 現在の天気
* **波の高さ・海水温**: 茅ヶ崎ヘッドランド（北緯: `35.318` / 東経: `139.410`）を基準点としたOpen-Meteo海洋モデル（25km〜50km四方のグリッドデータ）を使用。GitHub Actionsにより30分ごとに `data/weather_marine.json` へ自動保存。
...
- **海面・現在の気象データ**: Open-Meteo API（登録不要・無料。GitHub Actionsにより30分ごとに自動取得）
- **潮汐データ (サブ)**: Stormglass API（気象庁データ欠損時のフォールバック。GitHub Actionsにより日次取得）
