...
- **翌年ぶんの配置を忘れた場合の挙動**: `extract_daily_data.py` が当年の元データを見つけられず `update-daily-data.yml` が失敗（exit 1）します。このとき `moon_daily.json` は前年末の値のまま残るため、フロントは `date` フィールドが当日と一致するかを検証し、不一致なら採用せず "計算値" 表示へフォールバックします（古い月齢を "NASA" として出し続けると、潮回り＝大潮/中潮の判定まで連動して誤るため）。**表示が毎日 "計算値" のままになっていたら、この配置漏れを疑ってください。**
