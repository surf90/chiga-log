...
- **データ鮮度の明示**: 各データの生成時刻（`updated_at`）を基準に鮮度を判定し、更新が停止している（Actions停止・API障害等）場合や古いデータのときは、影響を受けるカードにだけ注記を表示します。誤って古い情報を最新と誤読させません（詳細は下記「データ鮮度の表示と堅牢化」）。
...
- **月齢・潮回り**: NASA SVS（Scientific Visualization Studio）の月齢から、当日JST正午の月齢と朔の日を1日とする陰暦日を算出。潮回りは陰暦日から判定。
- **満干潮・潮位**: 気象庁の湘南港（江の島）潮位表（`tide_data.json`）を優先的に使用。
...
- **表示範囲**: 2日分を横スクロールで表示します。潮汐グラフと波グラフのスクロール位置は連動します。
- **初期位置**: 開いた時点で「今」の少し手前までスクロールした状態になり、現在時刻を示すオレンジの縦帯と、これから先の変化がすぐ見えます。
- **Y軸の固定**: 横スクロールしても左右の目盛り（潮位 m・波高 m・周期 秒）は端に貼り付いたままなので、どの時間帯を見ていても数値を読み取れます。
- **右端のフェード**: グラフ右端がわずかに薄くなっているのは、横に続きがある（2日目のデータがある）ことを示すためです。
...
- **熱中症情報**: 環境省の公式CSVから神奈川県（都道府県コード `14`）の正式発表を取得し、発表中だけ「注意報・警報」の直下、「天気予報」の直上に専用カードを表示します。
  - 熱中症警戒アラート（フラグ `1`）は黄〜黄橙の枠と薄い黄色背景、熱中症特別警戒アラート（フラグ `3`）は濃いオレンジ枠で一段強く表示します。特別警戒の事前判定（フラグ `2`）は表示しません。
  - 前日17時・当日5時の通常警戒と、前日14時の特別警戒を対象日ごとに判定します。専用Actionsは各発表の25分・15分・5分前に取得を試み、起動遅延に備えます。先行取得できても公式発表時刻までは表示せず、既存の30分ワークフローも安全網として残します。
...
- **表示方法**: これから先の**3件は常に表示**し、それ以降を「残りの予想風を表示」で開閉します。残りが無い時間帯（夜間など）はボタン自体を表示しません。
...
- **セクション別の鮮度注記**: 潮汐・波・天気・風・警報・現在の気温･風の各データが、種別ごとの閾値（更新頻度に応じて個別設定）を超えて古い場合に「データが古い可能性」を各カードに注記します。生成時刻フィールドの表記差（`updated_at` / `fetchedAt` / `observed_at`）は内部で吸収するため、データ種別によらず同一ロジックで鮮度を判定します。GitHub Actionsのスケジュール遅延は種別ごとに独立して起きるため、あえてページ全体の警告バナーは持たず、影響を受けるカードだけに注記することで「他の情報まで古い」という誤認を避けています。
...
- 日次抽出は当日JST正午（03:00 UTC）のエントリを参照し、元データの `time` と配列位置が一致することも検証します
- 潮回りは月齢の四捨五入ではなく、月齢と当日内の朔時刻からJST基準の陰暦日を求めて判定します
- `moon_daily.json` には当日から35日ぶんのNASA由来潮回りも同梱します。Actionsの一時遅延やオフラインキャッシュで `date` が前日のままでも、当日キーがあれば潮回りはこのカレンダーを使用し、収録範囲外だけ平均朔望月による概算へ移ります
- 月齢の表示に "NASA" と出ていれば正常にNASAデータを参照中、"計算値" と出ていればフォールバック中です
- **翌年ぶんの配置を忘れた場合の挙動**: `extract_daily_data.py` が当年の元データを見つけられず `update-daily-data.yml` が失敗（exit 1）します。このとき `moon_daily.json` は前年末の値のまま残るため、フロントは古い月齢を採用せず "計算値" 表示へフォールバックします。潮回りは前年末に生成できた翌年カレンダーの範囲内ならNASA由来値を維持し、範囲外は概算になります。**表示が毎日 "計算値" のままになっていたら、この配置漏れを疑ってください。**
- なお、日次ジョブ（JST 0:05）はActionsの起動遅延で最大1時間半ほどずれることがあり、その間は当日ぶんが未生成のため "計算値" 表示になります（正常）
