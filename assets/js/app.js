...
async function calculateTide() {
    ...
    else if ((r >= 3 && r <= 6) || (r >= 12 && r <= 13) || (r >= 17 && r <= 20) || (r >= 26 && r <= 28)) tideType = "中潮";
    else if ((r >= 7 && r <= 9) || (r >= 21 && r <= 23)) tideType = "小潮";
    else if (r === 10 || r === 24) tideType = "長潮";
    else if (r === 11 || r === 25) tideType = "若潮";
    else tideType = "不明";

    const ageLabel = ageSource === "NASA" ? `月齢: ${age.toFixed(1)}` : `月齢: ${age.toFixed(1)} / 計算値`;
            ...
            plugins: {
                ...
                tooltip: {
                    callbacks: {
                        ...
                        label: c => hasHeightData ? c.parsed.y.toFixed(2) + ' m' : '潮位イメージ'
                    }
                }
            },
            scales: {
...
function drawWaveCombinedChart(canvasId, existingInstance, data) {
    ...
    const todayJstStartMs = Math.floor((Date.now() + JST_OFFSET_MS) / 86400000) * 86400000 - JST_OFFSET_MS;
        ...
        options: {
            ...
            scales: {
                ...
                yWave: {
                    ...
                    title: { display: false },
                    ticks: { padding: 0, maxTicksLimit: 4, callback: v => v.toFixed(1) },
                ...
                yPeriod: {
                    ...
                    title: { display: false },
                    ticks: { padding: 0, maxTicksLimit: 4, stepSize: 1, callback: v => Number.isInteger(v) ? v : null },
...
async function fetchJmaWarning() {
    ...
    try {
        const data = await fetchCached(warningUrl, 'cache_jma_warning', { store: 'local', ttlMs: 10 * 60 * 1000 });
...
async function fetchJmaForecast() {
    ...
    try {
        ...
        const tempMax = findTemp(nowJstDate, '09') ?? findTemp(tomorrowJstDate, '09');
        const todayMinRaw = findTemp(nowJstDate, '00');
        const tomorrowMin = findTemp(tomorrowJstDate, '00');
        const tempMin = (todayMinRaw != null && todayMinRaw !== tempMax) ? todayMinRaw : (tomorrowMin ?? todayMinRaw);
...
function renderWindForecast(entries) {
    ...
    entries.forEach(({ time, dir, speed }) => {
        const row = document.createElement('div');
        row.className = 'wind-row';
        const t = document.createElement('span'); t.className = 'wind-time'; t.textContent = time;
        const d = document.createElement('span'); d.className = 'wind-dir'; d.textContent = dir || 'データなし';
        const s = document.createElement('span'); s.className = 'wind-speed'; s.textContent = `${speed ?? '-'} m/s`;
        row.append(t, d, s);
        grid.appendChild(row);
    });
}
...
function updateWindForecastToggleLabel(isOpen) {
    ...
    btn.textContent = isOpen ? `予想風（${range}）を閉じる ▲` : `予想風（${range}）を表示 ▼`;
}
...
async function fetchWindForecast() {
    try {
        ...
        }).filter(item => item.h >= 4 && item.h <= 23 && item.ts >= cutoff && item.dateJst === todayJst).slice(0, 21);
...
async function fetchWeatherData(isManual = false) {
    ...
    try {
        ...
        const wmData = await fetchCached('data/weather_marine.json', 'cache_weather_marine');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (toast) toast.addEventListener('click', () => { fetchWeatherData(); hideToast(); });
    const currentTime = document.getElementById('current-time');
    if (currentTime) currentTime.addEventListener('click', () => fetchWeatherData(true));
});
